...
# render_template_string : 템플릿 파일(.html) 대신, 파이썬 문자열로 작성한 Jinja2 템플릿을 바로 렌더링해서 HTML로 반환하는 함수.
# make_response : Flask에서 반환할 Response(응답) 객체를 직접 만들어서 세밀하게 제어할 때 사용.
# redirect : 클라이언트를 다른 URL로 이동시키는 응답(리다이렉트) 을 만든다.
# url_for : 라우트(뷰 함수)의 “이름”으로 URL을 생성해주는 함수.
app = Flask(__name__);

# Cookie는 브라우저에 저장되는 작은 키-값 데이터이고, 서버가 클라이언트와 연결 상태를 유지하는 것 처럼 할 수 있다.
...
def set_cookie():
    ...
    # 먼저 "read_cookie 페이지로 이동"하는 redirect 객체를 만들고
    # 그 응답에 따라 쿠키를 추가한 뒤 브라우저에 돌려줌 
    resp = make_response(redirect(url_for("read_cookie")))          # url_for를 쓰면 함수를 호출하는 것이고 안쓰면 "/read_cookie"를 부르는 것
    resp.set_cookie(                # 브라우저에 쿠키 저장
        key="name",                 # 쿠키 이름
        value=name,                 # 사용자가 입력한 쿠키 값
        max_age=60*5,               # 쿠키 유효 시간 5분 뒤 만료(안주면 대체로 1년)
        httponly=True,              # JS에서 document.cookie로 접근 불가
        samesite="Lax",             # CSRF 공격 방지용
    )
    return resp                     # 쿠키가 포함된 응답을 브라우저로 반환
    #브라우저는 쿠키를 저장, redirect 요청에 따라 read_cookie로 다시 요청함


@app.get("/read_cookie")
def read_cookie():
    # 브라우저가 요청에 실어 보낸 모든 쿠키 중에서 내 서버가 만든 name 쿠키를 꺼냄
    # - 업승면 None 반환(첫방문/만료/삭제된 경우)
