...
# 귀무 : 교육수준과 흡연율간에 관계가 없다.     (독립이다. 연관성이 없다.)
# 대립 : 교육수준과 흡연율간에 관계가 있다.     (독립이 아니다. 연관성이 있다.)
import pandas as pd
...
# 판정 1 : 유의수준 : 0.05 > p:0.000818257 이므로 귀무가설이 기각
# 교육 수준과 흡연율간에 관계가 있다. smoke.csv(수집자료)는 우연히 발생된 자료가 아니다.

# 판정 2: chi2: 19.910, dof: 4, critical value: 9.49
...
# 판정: 예산된 기대도수와 관측값은 서로 관련이 있는가?
# 유의수준 0.05 > p value 0.00338이므로 귀무가설 기각
# 성별과 음료 선호는 서로 관련이 있다.
