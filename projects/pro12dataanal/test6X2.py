...
# 분포 비율 차이 검정
# 두 집단 이상에서 각 범주 집단간의 비율이 서로 동일한가를 검정하게 된다.
...
data = pd.read_csv('http://raw.githubusercontent.com/pykwon/python/refs/heads/master/testdata_utf8/survey_method.csv')
...
# 검정통계량이 작으면 p-value는 크다. 귀무 채택. 우연히 발생한 자료라고 할 수 있다.

print('------'*10)
# 동질성 검정 실습 2) 연령대별 sns 이용률의 동질성 검정
# 20대에서 40대까지 연령대 별로 서로 조금씩 그 특성이 다른 sns 서비스들에 대해 이용 현황을
# 조사한 자료를 바탕으로 연령대별로 홍보전략을 세우고자 한다.
# 연령대별로 이용 현황이 서로 동일한지 검정해보도록 하자

# 귀무 : 연령대별로 sns 서비스 별 이용률 현황은 동일하다
# 대립 : 연령대별로 sns 서비스 별 이용률 현황은 다르다.
...
# 판정 : p value : 1.167906 > 0.05이므로 귀무가설 기각
# 연령대 별로 sns 서비스 별 이용률 현황은 동일하지 않다. 의견을 인정함
...
print(sample_data.head(),' '.len(sample_data))

ctab3 = pd.crosstab(index=sample_data['age'], columns=data['service'])
print(ctab3)
