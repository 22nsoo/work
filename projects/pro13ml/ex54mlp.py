...
# 오차함수(loss function)는 L = (y-y^) 예측이 틀릴 수록 값이 커짐
# 그럼 미분은 왜 쓰나? 오차를 어떻게 줄일 지 즉, 오차가 줄어든 방향으로 w(가중치)를 갱신
# 전체 학습 과정을 보면 
# 1. 모델이 예측 -> 2. 오차 계산 -> 3. 미분(기울기 계산)
#             -> 4. 가중치 W를 계산 -> 5. 반복(1~4) - 역전파(back propagation)
# 이것이 MLP 학습
