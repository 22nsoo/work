...
print('x2, y2의 공분산 : ', np.cov(x2,y2)[0,1])             # x2, y2의 공분산 :  522.22

plt.plot(x2,y2,'o')
plt.show()

print()
# 데이터의 단위에 따라 패턴이 일치하더라도 공분산의 크기가 달라지므로 절대적 크기 판단이 어려움
# 공분산을 표준화해서 -1 0 1 범위로 만든 것이 상관계수(r)

print('x, y의 상관계수 : ', np.corrcoef(x,y))
print('x, y의 상관계수 : ', np.corrcoef(x,y)[0,1])              # 0행 1열 보기      0.8663

print('x, y의 상관계수 : ', np.corrcoef(x2,y2))
print('x, y의 상관계수 : ', np.corrcoef(x2,y2)[0,1])              # 0행 1열 보기    0.8663
