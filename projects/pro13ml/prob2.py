...
print('상관계수 : ', np.corrcoef(x1,y1)[0,1])
print('상관계수 : ', np.corrcoef(x1,y2)[0,1])
...
plt.scatter(x1,y1)
plt.plot(x1, model.slope*x1 + model.intercept, c='r')
plt.show()

model2 = stats.linregress(x1,y2)
print('기울기 : ', model2.slope)
print('절편 : ', model2.intercept)
print('p값 : ', model2.pvalue)

plt.scatter(x1,y2)
plt.plot(x1, model2.slope*x1 + model2.intercept, c='r')
plt.show()

x_new = 3  # 예측

# 운동 예측
pred_ex = model.slope * x_new + model.intercept
print(f"\n지상파 {x_new}시간 → 예상 운동시간: {pred_ex:.3f}")

# 종편 예측
pred_jong = model2.slope * x_new + model2.intercept
print(f"지상파 {x_new}시간 → 예상 종편시간: {pred_jong:.3f}")
