# 군집 분석: 데이터간의 유사도를 정의하고 그 유사도에 가까운 것부터 순서대로
# 합쳐가는 방법으로, 거리나 상관계수 등을 이용한다.
# 이는 비슷한 특성을 가진 개체를 그룹으로 만들고, 군집 분리 후 t-test, Anova 분석등을
...
df2 = pd.DataFrame(row_clusters, columns=['클러스터id','클러스터id','거리','클러스터멤버'])
print(df2)

# 클러스터의 계층 구조를 계통도(dendrogram)를 출력
